...
void Engine::CalculateStep()
{
	...
	for_each(exec_handle::execution::par, shipVector.begin(), shipVector.end(), [&](const shared_ptr<Ship> &it)
	{
	...
	for_each(exec_handle::execution::par, projectiles.begin(), projectiles.end(), [&](auto &projectile) {
...
void Engine::DoCollisions(vector<pair<mutex, list<Visual>>> &parallelBuffer, Projectile &projectile)
{
	...
	sort(exec_handle::execution::par_unseq, collisions.begin(), collisions.end());
